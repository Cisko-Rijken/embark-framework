...
Blockchain.prototype.get_address = function() {
  ...
  if (config.account.init) {
    ...
    this.new_address_if_none_exists()
...
Blockchain.prototype.new_address_if_none_exists = function(){
  var result = exec(this.list_command());
  if(result.output.indexOf("Fatal") !== -1){
    console.log("running: " + this.list_command());
    exec(this.init_command());
  }
}

Blockchain.prototype.get_address_indices = function(){
  this.new_address_if_none_exists()
  var result = exec(this.list_command());
      ,address_count = result.output.split().length-1
      ,indices = []
...
Blockchain.prototype.startChain = function(use_tmp) {
  var address_indices_string = this.get_address_indices().join();
  console.log("running: " + this.run_command(address_indices_string, use_tmp));
  exec(this.run_command(address_indices_string, use_tmp));
}
